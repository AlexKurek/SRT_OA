...
//    if ((fabs(d1.aznow - d1.azcmd) > 0.5 || fabs(d1.elnow - d1.elcmd) > 0.5) // was 1 should get within 0.5 deg
    if ((fabs(d1.aznow - d1.azcmd) >= 0.5 || fabs(d1.elnow - d1.elcmd) >= 0.5) // from steve.black@washburn.edu 27Nov17
...
//  printf("here cmd = -1\n");
   ...
   usbdev = open("/dev/ttyUSB0",O_RDWR,O_NONBLOCK);
      sleep(1);
      status = close(usbdev);
   if(d1.debug) printf("here stty usbdev %d status %d\n",usbdev,status);
...
// printf("acount %f azz %f d1.count %d %f usbdev %d\n",acount,azz,d1.azcount,azz * d1.azcounts_per_deg - d1.azcount,usbdev);
  ...
  if(d1.debug) printf(" move %d count %d %d\n",mm,count,'\n');
...
//  sleep(1);
  ...
  while(j==0 && i < 32)
  {
	  ...
	  if(d1.debug)  printf("here axis %d status %d ch %1c %x i = %d usbdev %d rstatus %d\n",axis,status,ch,ch,i,usbdev,rstatus);
	   usleep(10000);
	   if(status && i < 32)
		   resp[i] = ch; i++;
	  if(ch == 13 || ch == 10) j=99; // end 
  }
  status = i;
  usleep(100000);
...
//  for(i=0;i<status;i++) printf("i %d %c\n",i,resp[i]);
  ...
  if(d1.debug) printf("status %d im %d move mm %d sent %d recvd %d %12s rstatus %d azz %f ell %f\n",status,im,mm,count,ccount,&resp[im],rstatus,*az,*el);
  if(resp[im] == 'M')
  {
